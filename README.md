...
- **ストーリー** — 16人のライバルと1対1、4章立て。ステージは相手ごとに決まっています。
  一度倒した相手には、再戦のときに別の一言が出ます。
- **フリーラン** — 好きなステージを交通量ありで流します。
- **タイムアタック** — 好きなステージで1周のベストラップに挑みます。
- **ガレージ** — マシンの購入・6項目のチューン・カラー変更（3Dプレビュー付き）。
- **走行記録** — 総走行距離・依頼の達成／失敗・高速隊を振り切った回数・
  連行された回数・最高手配度・信号無視・PAに寄った回数・夜明けまで走った回数。
  条件を満たすと称号が付きます（14種）。

### 夜が明けていく
走っているあいだ、深夜2時から4時半へ時間が進みます。空の色・星・月・街灯・
映り込みが、1つの値（0〜1）から動きます。15分で明けきります。

### 一般道（側道）へ降りる
インターを降りてPAまで来たら、そのまま**まっすぐ進むと一般道**に出ます。
高速の外側・下を、コースに沿って一周している片側1車線の道です。

- **信号があります。** 赤で停止線を越えると手配度が上がります
- **制限速度は80km/h。** 高速の140km/hとは別基準です
- パトカーは本線から降りてこられないので、**逃げ込む先**にもなります
- 次のインターまで走れば、そこからまた本線へ上がれます
- PAが2か所以上あるコースだけに作られます（12コース中11コース）

横位置と高さは「両隣のPAの値をなめらかにつないだもの」です。
こうすると、落差のちがうPAからでも段差なく出入りできます。
...
| 積荷 | 横Gと減速Gの上限を超えない（荷が傷むと失敗） | ×1.9 |
| 要人送迎 | 一度もぶつけず、信号も守る | ×2.4 |
| 深夜便 | 手配度を1でも上げたら失敗 | ×1.6 |

### 路地
交差点の3つに1つ、一般道から直角に折れて入る行き止まりの道があります。
高速隊は入ってこないので、逃げ込む場所になります。
（この車に後退ギアはないので、転回できる幅にしてあります）

### 交差点と横断交通
赤信号のとき、交差点を横切る車が出ます。赤で突っ切ると出合い頭にぶつかります。
手配度が少し上がるだけでは、止まる側が時間を損するだけで守る理由になりません。
...
- **一般道まで追ってきます。** 逃げ場はパーキングエリアと路地の2つで、
  そこでは連行されません（パトカーは入ってきません）
...
- **一般道も、道路グラフなしで作っています。** ランプと同じ「本線の距離 s に対する
  横位置と高さ」で書けるので、走れる範囲を切り替えるだけで行き来できます。
  節点はパーキングエリアの中心で、そのあいだを両端が平らになるようにつなぎます。
  一定の深さにすると、落差を縮めたインターのPAから出られなくなります。
- **バンクのかかった区間では lat が傾いています（最大 lat.y=0.11）。**
  位置は pos + lat*u + up*h で決まるので、u が大きいほど高さがずれます。
  一般道は狙った落差から最大5.3mずれ、路地は46mで5mの坂になっていました。
  levelH() で lat の傾きぶんを打ち消して水平に保ちます。
- **路地は s を固定して u が動く帯です。** ここまでの道はすべて「本線に沿って
  進む」ものでしたが、路地は本線を横切る向きに延びます。同じ座標系のまま、
  走れる範囲の見張り方を u ではなく s にするだけで書けます。
- **点数の多い場所はまとめます。** 信号49・路地17のコースで、素朴に作ると
  メッシュが792個になりました。平らな塗りは1つの形にまとめ、同じ形が並ぶ
  ものはインスタンス化し、色の変わる信号だけインスタンスごとの色で塗ります。
- **走る場所の切り替えには、行きと帰りで別の条件を使います（ヒステリシス）。**
  同じ境目で判定すると、縁に沿って走っているあいだ毎フレーム行き来して車が暴れます。
...
### Vercel へのデプロイ
ビルド不要の静的サイトなので、リポジトリを繋ぐだけで配信できます。
`vercel.json` でキャッシュの効き方だけ指定しています
（vendor は長期・src と styles は短め・index.html は毎回検証）。
